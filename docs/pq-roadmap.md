...
**Status**: Partial proposals ([EIP-7851](https://eips.ethereum.org/EIPS/eip-7851), [EIP-8151](https://eips.ethereum.org/EIPS/eip-8151), [EIP-8164](https://github.com/prestwich/EIPs/blob/48a93b9a83d8cb3d78a983e4f9f60460651574ab/EIPS/eip-8164.md), all Draft). EIP-7851 and EIP-8151 cover EIP-7702-delegated EOAs; EIP-8164 covers protocol-level Ed25519 migration for any EOA but currently competes with EIP-8141's EOA default code (see caveat below).
...
- [EIP-8164](https://github.com/prestwich/EIPs/blob/48a93b9a83d8cb3d78a983e4f9f60460651574ab/EIPS/eip-8164.md) (Native Key Delegation for EOAs) — extends EIP-7702's `0xef01XX` designator space with `0xef0101` for Ed25519 native keys, plus a new tx type `0x05`. Once installed, all ECDSA paths (legacy tx types, 7702 auths, native-key auths) are protocol-rejected for the account. Unlike EIP-7851 it does not require a prior 7702 delegation, so it reaches the codeless EOA population that EIP-7851/EIP-8151 cannot. The crafted-signature creation path goes further by producing accounts whose ECDSA key *never existed*, providing a cryptographic guarantee of rootlessness rather than a procedural one.

EIP-7851 and EIP-8151 resolve revocability for 7702-delegated accounts. EIP-8164 extends the reach to any EOA, including codeless EIP-8141 EOAs that use [default code](/eoa-support), but only conditionally. As currently specced, EIP-8164 is a *parallel track* to EIP-8141 rather than a complementary layer: a `0xef0101` account has no code execution, so it cannot run EIP-8141 default code or originate frame transactions. The two mechanisms compete for the same code field on an EOA. EIP-8164 itself acknowledges this and defers the fix to a future combined designator that would carry both an embedded pubkey and a delegation address; once that lands, EIP-8164 becomes additive to EIP-8141's EOA story rather than alternative to it. Until then, an EOA picks one path or the other.

This is likely the last stage before full migration: all preceding infrastructure (PQ auth, key rotation, mempool protection) must be proven before the ECDSA fallback can be safely removed protocol-wide.

**Delivers**: elimination of the quantum-vulnerable weak link for 7702-delegated EOAs (via EIP-7851/EIP-8151, including the `ecRecover` patch for immutable contracts) and a path to permanent Ed25519 migration for any EOA (via EIP-8164). **Does not deliver**: a composable revocability path for EIP-8141 EOAs without a follow-up combined-designator EIP, recovery mechanisms for lost PQ keys, or elimination of the ECDSA fallback protocol-wide.
